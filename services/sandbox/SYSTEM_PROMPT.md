...
|Provider failover: when the model provider of a Codex or Claude Code thread has no capacity left, the deployment can move the thread to the other of the two harnesses with its history, and the thread stays there. `--no-failover` keeps the thread on its harness; `--failover` allows the switch again. After such a switch, `--codex` or `--claude` moves the thread back in the same sandbox, with the whole history. If a harness cannot read the converted history, the thread goes back to the harness and history from before the switch, and the reply says so.
