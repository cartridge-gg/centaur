...
|Provider failover: when the model provider of a Codex or Claude Code thread has no capacity left, the deployment can move the thread to the other of the two harnesses with its history, and the thread stays there. `--no-failover` keeps the thread on its harness; `--failover` allows the switch again. After such a switch, `--codex` or `--claude` goes back to that harness's own session from before the switch, in the same sandbox; the turns made on the other harness are not in that history.
