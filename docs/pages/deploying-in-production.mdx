...
new turns start on the other harness until then. Any authenticated caller,
including a sandbox, can read that record with `GET /api/provider-health`:
...
apiRs:
  providerHealth:
    ...
    intervalSecs: 60
``` A client that still asks for
the old harness gets the session on the new one; only a request with
`harness_explicit: true` restarts it on the old harness. Execution metadata
