...
#### Going back to the original session

A switch never changes or deletes the original native session: the new
harness gets a new, converted copy. Each switch adds a line to
`$CENTAUR_STATE_DIR/centaur-failover-journal.jsonl` with the session ids and
files on both sides. A request with `harness_explicit: true` for the harness
that the session left (for example `--codex` in Slack) goes back to that
harness's own session from before the switch, in the same sandbox. The turns
made on the other harness are not in its history, and the workspace keeps
their changes. This works while the sandbox of the switch serves the
session, because its state volume holds the original. After the sandbox is
replaced, the request restarts the session on that harness, as before.
