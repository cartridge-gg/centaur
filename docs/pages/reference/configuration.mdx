...
| `SLACKBOTV2_FILE_EVENT_WORKFLOW_NAME` | `slackbotv2.extraEnv`. | Workflow started (via `POST /api/workflows/runs`) when a matching Slack file event arrives. Unset disables the file-event trigger. Requires subscribing the Slack app to the file events being matched (`file_shared`, `file_change`) and `files:read` when the title-keyword filter is used. |
| `SLACKBOTV2_FILE_EVENT_TYPES` | `slackbotv2.extraEnv`. | Comma-separated Slack event types matched by the file-event trigger. Defaults to `file_shared,file_change`. |
| `SLACKBOTV2_FILE_EVENT_CHANNELS` | `slackbotv2.extraEnv`. | Optional comma-separated channel-id allowlist for the file-event trigger. Only enforced on events that carry a channel (`file_change` does not). Empty allows all channels. |
| `SLACKBOTV2_FILE_EVENT_TITLE_KEYWORDS` | `slackbotv2.extraEnv`. | Optional comma-separated, case-insensitive substrings matched against the file's `files.info` title/name/pretty_type before dispatching. Empty skips the lookup and dispatches every matching file event. |
| `SLACKBOTV2_FILE_EVENT_DEDUPE_SECONDS` | `slackbotv2.extraEnv`. | Time-buckets the dispatch idempotency key (`slack-file-event:<workflow>:<file_id>`). Unset: one run per file, ever — later edit events dedupe against the first run, which is expected to debounce internally. |
...
| `SESSION_SANDBOX_STATE_VOLUME_ENABLED`, `SESSION_SANDBOX_STATE_VOLUME_SIZE`, `SESSION_SANDBOX_STATE_VOLUME_STORAGE_CLASS_NAME`, `SESSION_SANDBOX_STATE_VOLUME_MOUNT_PATH` | `sandbox.stateVolume.{enabled,size,storageClassName,mountPath}`. | Persistent per-sandbox state: a PVC per session sandbox, kept across idle pauses and deleted with the sandbox, mounted at the mount path (`/home/agent/state`); the entrypoint links `~/.codex`, `~/.claude`, `~/uploads` and `~/branches` into it and receives `CENTAUR_STATE_DIR`. Needs a StorageClass that can provision the claim. Default off. |
| `apiRs.sandboxKeepaliveEnabled` / `SESSION_SANDBOX_KEEPALIVE_ENABLED`, `apiRs.sandboxKeepaliveMaxSecs` / `SESSION_SANDBOX_KEEPALIVE_MAX_SECS` | Helm values, default `false` / `604800` (7 days). | Session sandbox retention. Workflows call `ctx.session_keepalive(thread_key, key=..., generation=..., until=...)` to keep a session's sandbox (paused or running) past the max lifetime while the named work is open, one lease per key, and `ctx.stop_session_sandbox(...)` to close a lease and stop the sandbox once none remains. The retention record lives in `sessions.metadata.sandbox_retention`; its effective deadline is projected onto the sandbox as `centaur.ai/keep-until` / `centaur.ai/keep-hard-deadline`, which the max-lifetime reaper honours for at most the cap past the max lifetime. Refused without a max-lifetime sweep. |
| `SESSION_SANDBOX_RESUME_THREAD_ENABLED` | `apiRs.sandboxResumeThreadEnabled`, default `false`. | With the state volume on, sandboxes get `CENTAUR_CODEX_THREAD_PERSIST=1` and `CENTAUR_CLAUDE_SESSION_PERSIST=1`: the harness server writes its Codex thread id to `$CODEX_HOME/centaur-thread-id` and its Claude Code session id to `$CLAUDE_CONFIG_DIR/centaur-session-id`, and resumes that session on the next start, so a session paused and resumed keeps its conversation; a session that cannot be resumed starts fresh on the same pod. Refused without `SESSION_SANDBOX_STATE_VOLUME_ENABLED`. |
| `SESSION_PROVIDER_EXHAUSTED_MARKERS` | `apiRs.providerExhaustedMarkers`, default empty. | Comma-separated texts that mark an exhausted model provider in a harness error, for example the error code of a proxy whose whole account pool is rate limited. Sandboxes get `CENTAUR_PROVIDER_EXHAUSTED_MARKERS`. The harness server adds `params.centaur.providerExhausted` (reset time, signal) to such errors, the execution fails with `failure_class: provider_exhausted`, and api-rs records the reset time in `provider_health`. Native usage-limit errors count without a marker. |
| `SESSION_PROVIDER_HEALTH_URLS`, `SESSION_PROVIDER_HEALTH_INTERVAL_SECS`, `SESSION_PROVIDER_HEALTH_TOKEN` | `apiRs.providerHealth.{urls,intervalSecs}`; the token comes from the infra secret. Default: no checks, 60 s. | Health URLs of the model providers, as comma-separated `harness=url` pairs. api-rs sends `GET` with the bearer token on each interval. `{"exhausted": true, "reset_at": <Unix seconds>}` marks the provider exhausted in `provider_health` until then (15 minutes when `reset_at` is null); `{"exhausted": false}` clears the mark early. A failed check changes nothing. Checks are counted in `centaur_provider_health_probes_total{harness,result}`. The record is readable with `GET /api/provider-health`. |
| `SESSION_PROVIDER_FAILOVER_ENABLED`, `SESSION_PROVIDER_FAILOVER_MODELS`, `SESSION_PROVIDER_FAILOVER_THREAD_PREFIXES` | `apiRs.providerFailover.{enabled,models,threadPrefixes}`, default off. | Continue a Codex or Claude Code session on the other harness when its model provider has no capacity left (see [Provider Failover](/deploying-in-production#provider-failover)). Sandboxes get `CENTAUR_HARNESS_SWITCHING=1`, and user lines get a `centaur` directive. `models` is comma-separated `harness=model` pairs for the model after a switch; `threadPrefixes` limits failover to thread keys with these prefixes. Each switch appends `session.provider_failover`, sets `sessions.metadata.provider_failover`, and increments `centaur_session_provider_failovers_total{from,to,mode}`. |
