...
| `apiRs.mcpSessionsEnabled`, `CENTAUR_MCP_SESSIONS_ENABLED` | Helm value, default `false`. | Exposes the `centaur_session_*` MCP tools. They start and drive agent sessions in full harness sandboxes that run as the calling MCP principal. |
