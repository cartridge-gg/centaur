...
| `SESSION_SANDBOX_RESUME_THREAD_ENABLED` | `apiRs.sandboxResumeThreadEnabled`, default `false`. | With the state volume on, sandboxes get `CENTAUR_CODEX_THREAD_PERSIST=1`: the harness server writes its Codex thread id to `$CODEX_HOME/centaur-thread-id` and resumes that thread on the next start, so a session paused and resumed keeps its conversation; a thread that cannot be resumed starts fresh on the same pod. Refused without `SESSION_SANDBOX_STATE_VOLUME_ENABLED`. |
...
| `OPENAI_API_BASE_URL`, `OPENAI_RESPONSES_WEBSOCKET_URL`, `NANOCODEX_RESPONSES_TRANSPORT`, `NANOCODEX_RESPONSES_HISTORY`, `NANOCODEX_STORE_RESPONSES` | `sandbox.extraEnv`. | Route the Nanocodex harness's Responses transport at an OpenAI-compatible base such as a custom HTTPS Responses endpoint. `OPENAI_API_BASE_URL` sets the HTTPS Responses/tool base; `OPENAI_RESPONSES_WEBSOCKET_URL` overrides the WebSocket endpoint; `NANOCODEX_RESPONSES_TRANSPORT` selects `websocket` (default) or `https`; `NANOCODEX_RESPONSES_HISTORY` selects `incremental` (default) or `full-replay`; `NANOCODEX_STORE_RESPONSES` (`true`/`false`) controls server-side checkpoint retention. These match the upstream Nanocodex CLI flags; unset preserves the SDK defaults, and an invalid transport, history, or boolean fails the harness at startup. |
