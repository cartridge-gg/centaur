...
- **Questions to the user** — a Centaur thread cannot show Hermes's
  interactive prompts, so the harness answers them at once. The `clarify`
  tool gets an answer that tells the agent to put the question and its
  choices in its reply; the user answers in the thread. `sudo`, `secret`,
  and the desktop read tools get an empty answer, as on their own timeout.
  Without these answers, a `clarify` question held the turn for an hour.
...
| `HERMES_CONTINUE_SESSION_ID` | Explicit durable Hermes session key to resume; takes precedence over the saved key. A failed resume is an error, never a new conversation. |
| `CENTAUR_HERMES_SESSION_FILE` | Optional file for automatically saving and loading the durable conversation key. Use a persistent path unique to this Centaur session, alongside its persistent `HERMES_HOME`. |
...
The gateway's short `session_id` is a transient handle. Centaur reports and
persists the durable `stored_session_id`/`session_key` instead. Configure
`CENTAUR_HERMES_SESSION_FILE` on the session's persistent volume to recover
automatically after a gateway or harness restart. Draft sessions are not saved
until the first submitted turn; compaction updates the saved key. Keep this file
unique to the Centaur session, even when sharing other Hermes state. Missing
or corrupt durable history fails explicitly instead of silently starting over.
...
## Images and interruption

Uploaded local images and inline image data are attached through Hermes's image
RPCs before submitting the prompt. Hermes then uses its configured native vision
or image-description pipeline. Remote image URLs must be staged by the caller.
A failed attachment batch discards the gateway to avoid leaking images into the
next turn.

Interrupts preserve gateway events received before the RPC acknowledgement and
drain the parent turn's completion. If the gateway does not settle within 30
seconds, it is stopped so a late completion cannot end the next turn. Hermes
deliberately preserves background terminal jobs on interruption; run delegated
workers in the foreground if they must stop with the user turn.
